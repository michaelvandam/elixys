package Elixys.Views
{
	...
	public class Login extends Screen
	{
		...
		public function Login(screen:Sprite, xml:XML, attributes:Attributes = null, row:Boolean = false, inGroup:Boolean = false)
		{
			...
			super(screen, LOGIN_SCREEN, attributes, row, inGroup);
		}
		...
		protected function LoadLogin():void
		{
			...
			// Load the login
			var pAttributes:Attributes = new Attributes(0, 0, width, height);
			m_pLogin = new Screen(pContainer, LOGIN, pAttributes);
			...
			m_pLoginButton.addEventListener(Button.BUTTON_CLICK, OnLoginButton);
		...
		protected static const LOGIN_SCREEN:XML = 
			<frame background={Styling.APPLICATION_BACKGROUND} alignV="fill" alignH="fill">
				<rows gapV="0" border="false" heights="18%,64%" background={Styling.APPLICATION_BACKGROUND}>
					<frame />
					<columns id="login_container" gapH="0" widths="50%,40%" />
				</rows>
			</frame>;
		...
		protected static const LOGIN:XML =
			<rows gapV="5" heights="32%,7%,7%,7%,7%,3%,8%,29%">
				<frame />
				...
				<columns widths="50%,50%,20">
					...
					<button id="login" alignH="fill" enabled="false" useEmbedded="true"
							...
							skinup={getQualifiedClassName(login_signInBtncopy)}
							skindown={getQualifiedClassName(login_signInBtncopy)}
							skindisabled={getQualifiedClassName(login_signInBtncopy)}>
