...
def HandleGetState(sClientState, sRemoteUser):
    ...
    pUser = {"type":"user"}
    pUser.update(gDatabase.GetUser(sRemoteUser, sRemoteUser))
...
def GetPromptStateComponent(sClientState, sRemoteUser):
    ...
    if sClientState.startswith("PROMPT_CREATESEQUENCE"):
        ...
        return HandleGetStateSelectPromptCopySequence(pPromptState, sClientState, sRemoteUser)
    elif sClientState.startswith("PROMPT_DELETESEQUENCE"):
        return HandleGetStateSelectPromptDeleteSequence(pPromptState, sClientState, sRemoteUser)
...
def HandleGetStateSelectPromptCopySequence(pPromptState, sClientState, sRemoteUser):
    # Look up the sequence
    global gDatabase
    nSequenceID = int(sClientState.split(";")[0].split("_")[2])
...
def HandleGetStateSelectPromptDeleteSequence(pPromptState, sClientState, sRemoteUser):
    # Look up the sequence
    global gDatabase
    nSequenceID = int(sClientState.split(";")[0].split("_")[2])
    pSequence = gDatabase.GetSequence(sRemoteUser, nSequenceID)
...
def HandleGetStateRunSequencePromptUnitOperation(pPromptState, sClientState, sRemoteUser):
    ...
    nSequenceID = int(pRunStateComponents[1])
    nComponentID = int(pRunStateComponents[2])
    pComponent = GetComponent(sRemoteUser, nComponentID, nSequenceID)
...
def HandleGetComponent(sRemoteUser, sPath):
    ...
    return GetComponent(sRemoteUser, nComponentID, nSequenceID)
...
def HandleGetReagent(sRemoteUser, sPath):
    ...
    for nReagentID in pReagentIDs:
        pReagents["reagents"].append(GetReagent(sRemoteUser, int(nReagentID)))
    return pReagents
...
def HandlePostSelect(sClientState, sRemoteUser, pBody, nBodyLength):
    ...
    if sActionType == "BUTTONCLICK":
        if sActionTargetID == "VIEW":
            ...
            sClientState = "PROMPT_COPYSEQUENCE_" + str(nSequenceID) + ";" + sClientState
            gDatabase.UpdateUserClientState(sRemoteUser, sRemoteUser, sClientState)
            return HandleGet(sClientState, sRemoteUser, "/state")
        elif sActionTargetID == "DELETE":
            # Switch states to Prompt (Delete Sequence)
            sClientState = "PROMPT_DELETESEQUENCE_" + str(nSequenceID) + ";" + sClientState
            gDatabase.UpdateUserClientState(sRemoteUser, sRemoteUser, sClientState)
            return HandleGet(sClientState, sRemoteUser, "/state")
    ...
    if sManualRunStep == "CASSETTE":
        ...
        if sActionType == "BUTTONCLICK":
            if sActionTargetID == "BACK":
                ...
                DeleteComponent(sRemoteUser, nComponentID)
...
def HandlePostPrompt(sClientState, sRemoteUser, pBody, nBodyLength):
    ...
    if sClientState.startswith("PROMPT_CREATESEQUENCE"):
        if sActionTargetID == "CREATE":
            ...
            nSequenceID = gDatabase.CreateSequence(sRemoteUser, sEdit1, sEdit2, "Saved", 3, 10, 2)
        ...
        if sActionTargetID == "COPY":
            ...
            nSequenceID = int(sClientState.split(";")[0].split("_")[2])
            nSequenceID = CopySequence(sRemoteUser, nSequenceID, sEdit1, sEdit2, "Saved", 3, 10, 2)
        ...
        if sActionTargetID == "DELETE":
            ...
            nSequenceID = int(sClientState.split(";")[0].split("_")[2])
            gDatabase.DeleteSequence(sRemoteUser, nSequenceID)
...
def HandlePostComponent(sClientState, sRemoteUser, pBody, nBodyLength, sPath):
    ...
    if nComponentID != 0:
        ...
        sClientState = gDatabase.GetUserClientState(sRemoteUser, sRemoteUser)
        pClientStateComponents = sClientState.split(".")
        sClientState = pClientStateComponents[0] + "." + str(nSequenceID) + "." + str(nComponentID)
        gDatabase.UpdateUserClientState(sRemoteUser, sRemoteUser, sClientState)
...
def GetSequence(sRemoteUser, nSequenceID):
    ...
    for pComponent in pSequence["components"]:
        AddComponentDetails(sRemoteUser, pComponent, nSequenceID)

    # Return
    return pSequence

# Copies and existing sequence in the database
def CopySequence(sRemoteUser, nSequenceID, sName, sComment, sType, nCassettes, nReagents, nColumns):
    # Create the new sequence
    global gDatabase
    nNewSequenceID = gDatabase.CreateSequence(sRemoteUser, sName, sComment, sType, nCassettes, nReagents, nColumns)

    # Cassette loop
    for nCassette in range(1, nCassettes + 1):
        # Copy reagents
        for nReagent in range(1, nReagents + 1):
            pReagent = gDatabase.GetReagentByPosition(sRemoteUser, nSequenceID, nCassette, str(nReagent))
            gDatabase.UpdateReagentByPosition(sRemoteUser, nNewSequenceID, nCassette, str(nReagent), pReagent["available"], pReagent["name"],
                pReagent["description"])

        # Copy columns
        for nColumn in range(0, nColumns):
            sPosition = chr(ord("A") + nColumn)
            pColumn = gDatabase.GetReagentByPosition(sRemoteUser, nSequenceID, nCassette, sPosition)
            gDatabase.UpdateReagentByPosition(sRemoteUser, nNewSequenceID, nCassette, sPosition, pReagent["available"], pReagent["name"],
                pReagent["description"])
    ...
    for pComponent in pSequence["components"]:
         ...
         if pComponent["componenttype"] == "CASSETTE":
             Log("Update cassette: " + str(pComponent))
         else:
             Log("Add component: " + str(pComponent))

    # Return the ID of the new sequence
    return nNewSequenceID    

# Fetches a component from the database
def GetComponent(sRemoteUser, nComponentID, nSequenceID):
...
def UpdateComponent(sRemoteUser, nSequenceID, nComponentID, nInsertionID, pComponent):
    ...
    if pComponent != None:
        ...
        pDBComponent = GetComponent(sRemoteUser, nComponentID, nSequenceID)
...
# Deletes a component
def DeleteComponent(sRemoteUser, nComponentID):
    ...
    if nClientComponentID == nComponentID:
        ...
        pComponent = gDatabase.GetNextComponent(sRemoteUser, nComponentID)
        if pComponent == None:
            # This is the last component in the sequence so move to the previous component
            pComponent = gDatabase.GetPreviousComponent(sRemoteUser, nComponentID)
            if pComponent == None:
    ...
    gDatabase.DeleteComponent(sRemoteUser, nComponentID)

# Gets a reagent from the database
def GetReagent(sRemoteUser, nReagentID):
    # Fetch the reagent from the databse
    global gDatabase
    pReagent = gDatabase.GetReagent(sRemoteUser, nReagentID)
...
def AddComponentDetails(sRemoteUser, pComponent, nSequenceID):
    ...
    if pComponent["componenttype"] == "CASSETTE":
        ...
        pReagents = []
        for nReagentID in pReagentIDs:
            pReagents.append(GetReagent(sRemoteUser, int(nReagentID)))
        ...
        if pComponent["reagent"] != "":
            pReagents = gDatabase.GetReagentsByName(sRemoteUser, nSequenceID, pComponent["reagent"])
            if len(pReagents) > 0:
